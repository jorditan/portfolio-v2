...
export default function ProjectDetailCarousel({ images = [], title = "" }) {
  ...
  return (
    <div className="relative flex w-full flex-col gap-3">
      ...
      <div
        className="relative aspect-[16/10] sm:aspect-video w-full overflow-hidden rounded-xl border border-slate-300/60 bg-gray-100 hover:border-slate-400/90 dark:hover:border-white/40 shadow-md shadow-slate-300/40 backdrop-blur-sm transition-all duration-300 dark:border-white/10 dark:bg-gray-800 dark:shadow-black/30"
      ...
      >
        {images.map((img, index) => {
          ...
          return (
            <div
              key={index}
              className={`absolute md:p-4 inset-0 h-full w-full transition-opacity duration-500 ease-in-out ${
                isActive
                  ? "z-10 opacity-100"
